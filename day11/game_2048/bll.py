...
class GameCoreController:
    ...
    def empty_locatation_generate(self): # 找空白
        list_empty_location = self.__get__empty_location()
        loc = random.choice(list_empty_location)
        number = self._select_random_number()
        self.__map[loc[0]][loc[1]] = number

    def _select_random_number(self):
        return 4 if random.randint(1, 10) == 1 else 2

    def __get__empty_location(self):
        list_empty_location = []
        for l in range(len(self.__map)):
            for c in range(len(self.__map[0])):
                if self.__map[l][c] == 0:
                    list_empty_location.append((l, c))
